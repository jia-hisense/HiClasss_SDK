...
/**
 `BMMemoryCache` is a fast, thread safe key/value store similar to `NSCache`. On iOS it will clear itself
 ...
 Values will not persist after application relaunch or returning from the background. See <BMCache> for
 a memory cache backed by a disk cache.
 */

BMC_SUBCLASSING_RESTRICTED
@interface BMMemoryCache : NSObject <BMCacheProtocol, BMCacheObjectSubscripting>
 ...
 */
@property (nullable, copy) BMCacheObjectBlock willAddObjectBlock;
...
/**
 ...
 Calling synchronous methods on the cache within this callback will likely cause a deadlock.
 */
@property (nullable, copy) BMCacheObjectBlock willRemoveObjectBlock;
...
/**
 ...
 Calling synchronous methods on the cache within this callback will likely cause a deadlock.
 */
@property (nullable, copy) BMCacheObjectBlock didAddObjectBlock;
...
/**
 ...
 Calling synchronous methods on the cache within this callback will likely cause a deadlock.
 */
@property (nullable, copy) BMCacheObjectBlock didRemoveObjectBlock;
 ...
 */
@property (nullable, copy) BMCacheBlock didReceiveMemoryWarningBlock;
 ...
 */
@property (nullable, copy) BMCacheBlock didEnterBackgroundBlock;
 ...
 */
@property (class, strong, readonly) BMMemoryCache *sharedCache;
...
- (instancetype)initWithName:(NSString *)name operationQueue:(BMCacheOperationQueue *)operationQueue ttlCache:(BOOL)ttlCache NS_DESIGNATED_INITIALIZER;
 ...
 */
- (void)trimToCostAsync:(NSUInteger)cost completion:(nullable BMCacheBlock)block;
...
/**
 ...
 @param block A block to be executed concurrently after the cache has been trimmed, or nil.
 */
- (void)trimToCostByDateAsync:(NSUInteger)cost completion:(nullable BMCacheBlock)block;
 ...
 */
- (void)enumerateObjectsWithBlockAsync:(BMCacheObjectEnumerationBlock)block completionBlock:(nullable BMCacheBlock)completionBlock;
...
typedef void (^BMMemoryCacheBlock)(BMMemoryCache *cache);
typedef void (^BMMemoryCacheObjectBlock)(BMMemoryCache *cache, NSString *key, id _Nullable object);
typedef void (^BMMemoryCacheContainmentBlock)(BOOL containsObject);

@interface BMMemoryCache (Deprecated)
- (void)containsObjectForKey:(NSString *)key block:(BMMemoryCacheContainmentBlock)block __attribute__((deprecated));
- (void)objectForKey:(NSString *)key block:(nullable BMMemoryCacheObjectBlock)block __attribute__((deprecated));
- (void)setObject:(id)object forKey:(NSString *)key block:(nullable BMMemoryCacheObjectBlock)block __attribute__((deprecated));
- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost block:(nullable BMMemoryCacheObjectBlock)block __attribute__((deprecated));
- (void)removeObjectForKey:(NSString *)key block:(nullable BMMemoryCacheObjectBlock)block __attribute__((deprecated));
- (void)trimToDate:(NSDate *)date block:(nullable BMMemoryCacheBlock)block __attribute__((deprecated));
- (void)trimToCost:(NSUInteger)cost block:(nullable BMMemoryCacheBlock)block __attribute__((deprecated));
- (void)trimToCostByDate:(NSUInteger)cost block:(nullable BMMemoryCacheBlock)block __attribute__((deprecated));
- (void)removeAllObjects:(nullable BMMemoryCacheBlock)block __attribute__((deprecated));
- (void)enumerateObjectsWithBlock:(BMMemoryCacheObjectBlock)block completionBlock:(nullable BMMemoryCacheBlock)completionBlock __attribute__((deprecated));
