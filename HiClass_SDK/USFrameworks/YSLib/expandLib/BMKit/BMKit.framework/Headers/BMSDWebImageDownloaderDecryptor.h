...
typedef NSData * _Nullable (^BMSDWebImageDownloaderDecryptorBlock)(NSData * _Nonnull data, NSURLResponse * _Nullable response);
...
*/
@protocol BMSDWebImageDownloaderDecryptor <NSObject>
...
*/
@interface BMSDWebImageDownloaderDecryptor : NSObject <BMSDWebImageDownloaderDecryptor>

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
- (nonnull instancetype)initWithBlock:(nonnull BMSDWebImageDownloaderDecryptorBlock)block;

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
+ (nonnull instancetype)decryptorWithBlock:(nonnull BMSDWebImageDownloaderDecryptorBlock)block;

@end

/// Convenience way to create decryptor for common data encryption.
@interface BMSDWebImageDownloaderDecryptor (Conveniences)

/// Base64 Encoded image data decryptor
@property (class, readonly, nonnull) BMSDWebImageDownloaderDecryptor *base64Decryptor;
