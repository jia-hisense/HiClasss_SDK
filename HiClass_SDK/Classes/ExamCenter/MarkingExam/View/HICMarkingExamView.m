...
@interface HICMarkingExamView ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong)NSMutableArray *dataArr;
@end
@implementation HICMarkingExamView
- (instancetype)init {
    if (self = [super init]) {
        [self createUI];
    }
...
- (void)createUI {
    ...
    self.frame = CGRectMake(HIC_ScreenWidth, 0 , HIC_ScreenWidth, HIC_ScreenHeight - HIC_NavBarAndStatusBarHeight - 50 - 6);
...
- (void)configTableView {
    ...
    [self registerClass:[HICExamCell class] forCellReuseIdentifier:markCell];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    HICExamCell *cell = (HICExamCell *)[tableView dequeueReusableCellWithIdentifier:markCell];
    if (cell == nil) {
        cell = [[HICExamCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:markCell];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    reportModel.tab = NSLocalizableString(@"reviewing", nil);
