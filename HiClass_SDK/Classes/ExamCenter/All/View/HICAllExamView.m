...
@interface HICAllExamView ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong)NSMutableArray *dataArr;
@end
@implementation HICAllExamView
- (instancetype)init {
    if (self = [super init]) {
        [self createUI];
    }
...
- (void)createUI {
    ...
    self.frame = CGRectMake(HIC_ScreenWidth *4, 0 , HIC_ScreenWidth, HIC_ScreenHeight - HIC_NavBarAndStatusBarHeight - 50 - 6);
...
- (void)configTableView {
    ...
    [self registerClass:[HICExamCell class] forCellReuseIdentifier:allCellId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    HICExamCell *cell = (HICExamCell *)[tableView dequeueReusableCellWithIdentifier:allCellId];
    if (cell == nil) {
        cell = [[HICExamCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:allCellId];
    }
