...
#define Detail_Evaluate_Cell_Tag 60000

@implementation OTPListDetailEvaluateCell
...
-(UIView *)createViewWithFrame:(CGRect)frame dataModel:(OfflineTrainingListModel *)model index:(NSInteger)index {
    ...
    backClickBut.tag = Detail_Evaluate_Cell_Tag + index;
    [backClickBut addTarget:self action:@selector(clickEvaluateBut:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [view addSubview:but];
    but.tag = Detail_Evaluate_Cell_Tag + index;
...
-(void)clickEvaluateBut:(UIButton *)but {
    DDLogDebug(@"------- 评价 --------- %ld ", (long)but.tag - Detail_Evaluate_Cell_Tag);
    NSInteger index = but.tag - Detail_Evaluate_Cell_Tag;
