//
//  OTPListDetailQuestionnaireCell.m
//  HiClass
//
...
#define Detail_Questionnaire_Cell_Tag 90000

@implementation OTPListDetailQuestionnaireCell
...
-(UIView *)createViewWithFrame:(CGRect)frame dataModel:(OfflineTrainingListModel *)model index:(NSInteger)index {
    ...
    backClickBut.tag = Detail_Questionnaire_Cell_Tag + index;
    [backClickBut addTarget:self action:@selector(clickQuestionnaireBut:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [view addSubview:but];
    but.tag = Detail_Questionnaire_Cell_Tag + index;
    [but addTarget:self action:@selector(clickQuestionnaireBut:) forControlEvents:UIControlEventTouchUpInside];
    ...
    // 赋值
    [self setValueWith:nil timeLabel:timeLabel but:but titleLabel:titleLabel commitLabel:nil signTypeLabel:nil againSignBut:nil backBut:backClickBut model:model];

    return view;

}

-(void)clickQuestionnaireBut:(UIButton *)but {
    DDLogDebug(@"问卷的点击事件 ----------- %ld", (long)but.tag - Detail_Questionnaire_Cell_Tag);
    NSInteger index = but.tag - Detail_Questionnaire_Cell_Tag;
