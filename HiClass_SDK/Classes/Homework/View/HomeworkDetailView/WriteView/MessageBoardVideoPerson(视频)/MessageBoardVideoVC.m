...
@interface MessageBoardVideoVC ()<AVCaptureFileOutputRecordingDelegate,CAAnimationDelegate>
...
// 拍摄时间短的提示
@property (nonatomic, strong) UILabel *shortTimeWarningLab;
//聚焦
@property (nonatomic, strong) UIImageView *focusImage;
//进度
@property (nonatomic, strong) CAShapeLayer *progressLayer;
//视频预览
@property (nonatomic, strong) MessageBoardVideoPlayer *player;
//图像预览层，实时显示捕获的图像
...
//保存当前视频时间
@property (nonatomic, copy) NSString *videoDur;
@end

@implementation MessageBoardVideoVC
...
- (UILabel *)tipLab{
   if (!_tipLab) {
      ...
      _tipLab.text = NSLocalizableString(@"holdShooting", nil);
...
// 时间短的提示
-(UILabel *)shortTimeWarningLab {
    
    if (!_shortTimeWarningLab) {
        _shortTimeWarningLab = [[UILabel alloc] init];
        _shortTimeWarningLab.font = [UIFont systemFontOfSize:14];
        _shortTimeWarningLab.textColor = [UIColor whiteColor];
        _shortTimeWarningLab.textAlignment = NSTextAlignmentCenter;
        _shortTimeWarningLab.text = NSLocalizableString(@"videoShootingShouldNotLessThanThreeSeconds", nil);
        [self.view addSubview:_shortTimeWarningLab];
    }
    return _shortTimeWarningLab;
}
...
#pragma mark 视频预览
- (MessageBoardVideoPlayer *)player{
   if (!_player) {
      _player = [[MessageBoardVideoPlayer alloc]init];
...
//      _player.frame = CGRectMake(0, 0, CGRectGetWidth(self.bgView.frame), CGRectGetHeight(self.bgView.frame));
       _player.frame = CGRectMake(0, 0, HIC_ScreenWidth, HIC_ScreenHeight);
      _player.hidden = YES;
...
- (void)configUI{
   ...
   self.centerY = self.view.height - 100 - HIC_BottomHeight;
   //背景
//   self.bgView.frame = CGRectMake(0, kIsIphoneX?kHMBStatusBarHeight:0, self.view.width, self.view.height - kHMBBottomSafe);
    self.bgView.frame = CGRectMake(0, 0, self.view.width, self.view.height);
   
   //摄像头切换
   self.cameraBtn.frame = CGRectMake(self.view.width - 15 - 40,(HIC_isIPhoneX?HIC_StatusBar_Height:0) + 23, 40, 40);
   ...
   self.backBtn.center = CGPointMake(HIC_ScreenWidth/4, self.centerY);
   
   //提示
   self.tipLab.frame = CGRectMake(0, self.takeImage.Y - 20 - 30, self.view.width, 20);
    self.shortTimeWarningLab.frame = CGRectMake(0, self.takeImage.Y-20-30, self.view.width, 20);
    self.shortTimeWarningLab.hidden = YES; // 上来隐藏
...
- (void)configCamera{
   ...
   if (error) {
      DDLogDebug(@"视频输入对象,错误原因：%@",error.localizedDescription);
      return;
   }
   ...
   if (error) {
      DDLogDebug(@"音频输入对象时出错，错误原因：%@",error.localizedDescription);
      return;
   }
...
- (void)stopLayout{
   ...
   CGFloat centerX = HIC_ScreenWidth/4;
...
- (void)recordingLayout{
   ...
   if ([self.movieOutPut isRecording]) {
      ...
      [UIView animateWithDuration:0.01 animations:^{
         ...
         animation.toValue = @(1);
         animation.duration = self.maxSeconds-0.26;
...
- (void)takePic{
   ...
   [self.imageOutPut captureStillImageAsynchronouslyFromConnection:connection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
      ...
      if (!imageDataSampleBuffer) {
         DDLogDebug(@"获取图片错误：%@",error.description);
         return ;
      }
   ...
   if (self.isVideo) {
      ...
      if (self.isSave) {//保存到系统
         ...
         [[ALAssetsLibrary new] writeVideoAtPathToSavedPhotosAlbum:[NSURL fileURLWithPath:self.tempPath] completionBlock:^(NSURL *assetURL, NSError *error) {
            ...
            if (error) {
               DDLogDebug(@"保存视频到相簿过程中发生错误，错误信息：%@",error.localizedDescription);
            }else{
               //处理视频
      ...
      if (self.isSave) {//保存到系统
         ...
         [[ALAssetsLibrary new] writeImageToSavedPhotosAlbum:[self.bgView.image CGImage] orientation:(ALAssetOrientation)self.bgView.image.imageOrientation completionBlock:^(NSURL *assetURL, NSError *error) {
            if (error) {
               DDLogDebug(@"保存图片到相簿过程中发生错误，错误信息：%@",error.localizedDescription);
            }else{
               //回调
               if (weakSelf.finishBlock) {
                  weakSelf.finishBlock(weakSelf.bgView.image, @"0");
               }
               //返回
         ...
         //回调
         if (self.finishBlock) {
            self.finishBlock(self.bgView.image, @"0");
         }
         //返回
...
- (void)dealVideoWithPath:(NSString *)path{
   ...
   if ([presets containsObject:AVAssetExportPresetMediumQuality]) {
      ...
      //压缩后的文件路径
       NSString *videoName = [HICCommonUtils getTimeWithZone];
      NSString *dealPath = [kHMBPath_video stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4", videoName]];
...
//       self.videoDur = [HMBBaseTools createTimeStringWithSeconds:CMTimeGetSeconds(avAsset.duration)];
         ...
         switch ([dealSession status]) {
            ...
            case AVAssetExportSessionStatusCompleted://完成
            {
               dispatch_async(dispatch_get_main_queue(), ^{
                  ...
                  DDLogDebug(@"\n原始文件大小：%f M\n压缩后视频大小：%f M ---- 时间：%lf",[weakSelf getVideoSizeWithPath:weakSelf.tempPath],[weakSelf getVideoSizeWithPath:dealPath], CMTimeGetSeconds(avAsset.duration));
                  //回调
                  if (weakSelf.finishBlock) {
                     weakSelf.finishBlock(dealPath, self.videoDur);
                  }
                  //返回
                  [weakSelf btnAction:weakSelf.backBtn];
               });
...
- (void)changeDeviceBlock:(PropertyChangeBlock)block{
   ...
   if ([captureDevice lockForConfiguration:&error]) {
      ...
      DDLogDebug(@"设置设备属性过程发生错误，错误信息：%@",error.localizedDescription);
   }
}
...
-(void)showShortTimeWarn {
    ...
    self.shortTimeWarningLab.hidden = NO;
    
    // 过一段时间隐藏提示
    [self performSelector:@selector(hiddenShortTimeWarn) withObject:nil afterDelay:1];
    
}
-(void)hiddenShortTimeWarn {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self btnAction:self.cancelBtn];
        
        self.shortTimeWarningLab.hidden = YES;
    });
}
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    //
//    CGFloat second = CMTimeGetSeconds(captureOutput.recordedDuration);
//    self.videoDur = [HMBBaseTools createTimeStringWithSeconds:second>15?15:second];
   //判断录制时间
   if (CMTimeGetSeconds(captureOutput.recordedDuration) < 3) {//拍照
      
//      [self takePic]; 拍照的功能，时间短
       [self showShortTimeWarn]; // 显示提示
   }else{//视频
