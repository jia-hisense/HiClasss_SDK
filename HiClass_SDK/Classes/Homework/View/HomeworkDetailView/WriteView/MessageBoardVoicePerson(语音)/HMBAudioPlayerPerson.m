...
@interface HMBAudioPlayerPerson ()<AVAudioPlayerDelegate>
...
@implementation HMBAudioPlayerPerson

#pragma mark - 实例化
+ (instancetype)shareInstance {
    static HMBAudioPlayerPerson *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[HMBAudioPlayerPerson alloc] init];
...
#pragma mark - 设置代理
- (void)setDelegate:(id<HMBAudioPlayerPersonDelegate>)delegate {
...
- (void)pauseAudio {
    if (_player) {
        ...
        if ([self.delegate respondsToSelector:@selector(didAudioPlayerPausePlay:)]) {
            [self.delegate didAudioPlayerPausePlay:self.playMark];
        }
    }
}
#pragma mark - stop
...
- (void)stopAudio {
    ...
    if ([self.delegate respondsToSelector:@selector(didAudioPlayerStopPlay:error:)]) {
        [self.delegate didAudioPlayerStopPlay:self.playMark error:error];
    }
    _player = nil;
    self.playMark = nil;
    _isStop = YES;
...
- (void)preparePlayAudio{
    ...
    NSString *viceoPath = [self getAudioFileName];
    _isStop = NO;
    if (viceoPath.length > 0) {
        //不随着静音键和屏幕关闭而静音
        //设置锁屏仍能继续播放
...
//        } else {//不是上次播放的录音
            ...
            DDLogDebug(@"语音路径：%@",viceoPath);
            
            //拼接WAV文件路径
            NSString *wavfile = [HICCommonUtils getFilePathWithName:viceoPath type:HMBMessageFileType_wav];
            //获取AMR文件路径
            NSString *amrFile = [HICCommonUtils getFilePathWithName:viceoPath type:HMBMessageFileType_amr];

            //判断本地是否存在AMR
            if (![[NSFileManager defaultManager] fileExistsAtPath:amrFile]) {
                //下载amr
                if ([viceoPath hasPrefix:@"https://"] || [viceoPath hasPrefix:@"http://"]) {
                    dispatch_async(dispatch_get_global_queue(0, 0), ^{
                        NSURL *url = [NSURL URLWithString:viceoPath];
                        NSData *voiceData = [NSData dataWithContentsOfURL:url];
                        dispatch_async(dispatch_get_main_queue(), ^{
                            if (!_isStop) {
                                [self startPlayAudioWithWavURL:viceoPath andWavData:voiceData];
                            }
                ...
                if (![[NSFileManager defaultManager] fileExistsAtPath:wavfile]) {
                    //不存在此条语音进行转换 AMR ——> WAV
                    [EMVoiceConverter amrToWav:amrFile wavSavePath:wavfile];
                }
                //开始播放
...
- (void)startPlayAudioWithWavFile:(NSString *)wavFile{
    ...
    self.playMark = [HICCommonUtils getNameWithUrl:wavFile];
    //开启距离感应
    ...
    //播放开始回调
    if ([self.delegate respondsToSelector:@selector(didAudioPlayerBeginPlay:)]) {
        [self.delegate didAudioPlayerBeginPlay:self.playMark];
    }
    ...
    if (!_player) {
        //播放错误
        if ([self.delegate respondsToSelector:@selector(didAudioPlayerStopPlay:error:)]) {
            [self.delegate didAudioPlayerStopPlay:self.playMark error:[error description]];
        }
        //播放下一条
        [self playNext];
    }
...
- (void)startPlayAudioWithWavURL:(NSString *)wavURL andWavData:(NSData *)data {
    
    self.playMark = [HICCommonUtils getNameWithUrl:wavURL];
    //开启距离感应
    [self changeProximityMonitorEnableState:YES];
...
- (NSString *)getAudioFileName{
    ...
    if (self.audioArr.count >= _playingIndex && _playingIndex != 0) {
        id name = [self.audioArr objectAtIndex:_playingIndex-1];
