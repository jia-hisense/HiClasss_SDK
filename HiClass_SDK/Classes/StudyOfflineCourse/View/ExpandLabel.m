...
#define PackUpStr   [NSString stringWithFormat:@" %@ ",NSLocalizableString(@"packUp", nil)]
#define ExpandStr  [NSString stringWithFormat:@"... %@",NSLocalizableString(@"develop", nil)]
//字体
...
- (void)setExpandAtt:(NSString *)text YYLabelW:(CGFloat)YYLabelW MaxLineNum:(NSInteger)maxLineNum font:(UIFont*)font color:(UIColor *)color LineSpace:(CGFloat)lineSpace{
    ...
    DDLogDebug(@"高度====%f=====%f",_maxHeight,_trueHeight);
...
- (NSAttributedString *)appendAttriStringWithFont:(UIFont *)font {
    ...
    NSString *appendText = PackUpStr;
    NSMutableAttributedString *append = [[NSMutableAttributedString alloc] initWithString:appendText attributes:@{NSFontAttributeName : font, NSForegroundColorAttributeName : HexRGB(0x00BED7)}];
...
- (NSAttributedString *)getShowMoreAtt{
    ...
    NSMutableAttributedString *text = [[NSMutableAttributedString alloc] initWithString:ExpandStr];
    
    NSRange expandRange = [text.string rangeOfString:ExpandStr];
    
    [text addAttribute:NSForegroundColorAttributeName value:HexRGB(0x00BED7) range:expandRange];
...
//
    //添加点击事件
    YYTextHighlight *hi = [YYTextHighlight new];
    [text yy_setTextHighlight:hi range:[text.string rangeOfString:ExpandStr]];
