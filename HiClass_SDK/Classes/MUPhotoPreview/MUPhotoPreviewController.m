...
#pragma clang diagnostic pop
    ...
    self.title = [NSString stringWithFormat:@"%lu/%lu ",(unsigned long)self.currentIndex+1,(unsigned long)totalCount];
...
-(MUPhotoPreviewView *)carouselView{
    if (!_carouselView) {
        ...
        _carouselView.doneUpdateCurrentIndex = ^(NSUInteger index) {
            ...
            weakSelf.customTitleLabel.text = [NSString stringWithFormat:@"%lu/%lu ",(unsigned long)index+1,(unsigned long)totalCount];
...
- (void)setFetchResult:(PHFetchResult *)fetchResult{
    ...
    if (fetchResult.count > 0) {
        ...
        self.title = [NSString stringWithFormat:@"%lu/%lu ",(unsigned long)_currentIndex+1,(unsigned long)fetchResult.count];
    }
}
...
- (void)setModelArray:(NSArray *)modelArray{
    ...
    if (modelArray.count >0) {
        ...
        self.title = [NSString stringWithFormat:@"%lu/%lu ",(unsigned long)_currentIndex+1,(unsigned long)modelArray.count];
    }
