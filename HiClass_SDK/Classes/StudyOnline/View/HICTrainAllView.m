...
static NSString *trainAllId = @"trainAllId";
@interface HICTrainAllView()<UITableViewDelegate,UITableViewDataSource>

@end
@implementation HICTrainAllView
...
- (void)configTableView{
    ...
    [self registerClass:[HICOnlineTrainCell class] forCellReuseIdentifier:trainAllId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    HICOnlineTrainCell *cell = (HICOnlineTrainCell *)[tableView dequeueReusableCellWithIdentifier:trainAllId];
    if (cell == nil) {
        cell = [[HICOnlineTrainCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:trainAllId];
    }
    cell.isAll = YES;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    reportModel.tab = NSLocalizableString(@"all", nil);
    ...
    if (model.trainType == 1) {//线上培训
        ...
        if (model.status == HICTrainInProgress || model.status == HICTrainFinished) {
            ...
            if (![NSString isValidStr:model.trainAssigner]) {
