...
static NSString *trainProgressId = @"trainProgressId";
@interface HICTrainInProgressView()<UITableViewDelegate,UITableViewDataSource>

@end
@implementation HICTrainInProgressView
- (instancetype)init {
    if (self = [super init]) {
        [self createUI];
    }
...
- (void)createUI{
    self.frame = CGRectMake(0, 0, HIC_ScreenWidth, HIC_ScreenHeight - HIC_NavBarAndStatusBarHeight - 44 -6);
...
- (void)configTableView{
    ...
    [self registerClass:[HICOnlineTrainCell class] forCellReuseIdentifier:trainProgressId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    HICOnlineTrainCell *cell = (HICOnlineTrainCell *)[tableView dequeueReusableCellWithIdentifier:trainProgressId];
    if (cell == nil) {
        cell = [[HICOnlineTrainCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:trainProgressId];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    reportModel.tab = NSLocalizableString(@"ongoing", nil);
    reportModel.mediaid = detailVC.trainId;
    reportModel.tasktype = [NSNumber numberWithInteger:HICReportTrainType];
    reportModel.trainmode = [NSNumber numberWithInteger:model.trainType];
    ...
    if (model.trainType == 1) {//线上培训
        ...
        if ((model.registChannel == 1 && model.registerId > 0) || model.registChannel == 2 ) {//已报名||指派
